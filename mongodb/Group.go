...
func Group() {
	...
	groupContract := bson.D{{"$group", bson.D{{"_id", bson.D{{"contract", "$contract"}}}, {"token", bson.D{{"$push", "$tokenId"}}}}}}
	...
	cursor, err := collection.Aggregate(context.TODO(), mongo.Pipeline{groupContract}, opt)
	if err != nil {
	...
	// declare interface, _id cannot be primitive ObjectID type
	var nfts []interface{}
